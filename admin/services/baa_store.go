...
// BAAStore persists BAA agreements in Postgres.
...
// Sign inserts a new BAA record for the tenant and agreement version.
// Uses ON CONFLICT to replace any existing record for the same (tenant, version) pair.
func (s *BAAStore) Sign(ctx context.Context, tenantID, userID, name, email, version, ip string) (*BAARecord, error) {
	if tenantID == "" || userID == "" || name == "" || email == "" {
		return nil, fmt.Errorf("tenantID, userID, name, and email are required")
	}
	if version == "" {
		version = "v1.0"
	}
...
// GetActive returns the active BAA record for the tenant, or nil if none exists.
